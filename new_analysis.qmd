---
title: "COMPREHENSIVE STOCK ANALYSIS SCRIPT"
---
```{r}
#| warning: false
#| echo: false
...
# ===============================================================================
cat("Last Updated \n")
cat(today_str <- format(Sys.Date(), "%Y_%m_%d"))
# ===============================================================================

cat("Loading data...\n")
stocks <- fread("data/Universe.csv")
...
cat("\nFirst few rows:\n")
as.data.table(t(head(stocks,2)),keep.rownames = T) %>% gt()
...
numeric_cols <- setdiff(colnames(stocks),c("Symbol","Name","Ind_Group_RS","ETF/Closed_End_Fund","Timeliness_Rating",
                                           ...
                                           "Order", "Current_Price", "%_New_Highs_in_Group", "Ind_Grp_Rnk_6_Mo_Ago", 
                                           "Debt_%", "Number_of_Stocks", "52-Wk_Low", "52-Wk_High", "Ind_Grp_Rnk_3_Mo_Ago", 
                                           "Ind_Grp_Rnk_Last_Week", "Trl_26_Wk_%_Perf_vs_S&P_500", "%_Chg_YTD", 
                                           "EPS_Trailing_4_Qtrs", "RS_Rating", "Market_Cap_(mil)", "Shares_(1000s)", 
                                           "Vol_%_Chg_vs_50-Day", "EPS_Rating", "Ind_Group_Rank", "10_Day_>_21_Day_>_50_Day", 
                                           "Price_$_Chg", "Volume_(1000s)", "Daily_Closing_Range", "RS_Line_Within_5%_of_New_High", 
                                           "50-Day_>_150-Day_>_200-Day", "Weekly_Closing_Range", "Current_day's_Volume_greater_than_previous_5_days'_Volume", 
                                           "Current_day's_Volume_greater_than_previous_10_days'_Volume", 
                                           "Current_day's_Volume_greater_than_previous_20_days'_Volume",
                                           "Sales Accel 2 Qtrs","Company Description","%_New_Highs_in_Group",
                                           "EPS Trl 4Q Gtr EPS 4 Yrs Ago","Industry Name","State","Debt_%","52-Wk_Low","52-Wk_High",
                                           "Ind_Grp_Rnk_3_Mo_Ago","Ind_Grp_Rnk_Last_Week"
                                           ))
...
sector_analysis <- stocks_filtered %>%
  ...
  summarise(
    Num_Stocks = n(),
    Avg_RS_Rating = round(replace_na(mean(RS_Rating, na.rm = TRUE), 0),1),#round(mean(RS_Rating, na.rm = TRUE), 1),
    #Avg_YTD_Return = round(replace_na(mean(`%_Chg_YTD`, na.rm = TRUE), 0), 1),
    Avg_YTD_Return = round(replace_na(mean(as.numeric(`%_Chg_YTD`[`%_Chg_YTD` != ""]), na.rm = TRUE), 0), 1),
...
ytd_stats <- stocks_filtered %>%
  ...
  summarise(
    ...
    Best_Performer = round(max(as.numeric(`%_Chg_YTD`[`%_Chg_YTD` != ""]), na.rm = TRUE), 2),
    Worst_Performer = round(min(as.numeric(`%_Chg_YTD`[`%_Chg_YTD` != ""]), na.rm = TRUE), 2),
    #Median_Return = round(median(`%_Chg_YTD`, na.rm = TRUE), 2),
    Median_Return = round(replace_na(mean(as.numeric(`%_Chg_YTD`[`%_Chg_YTD` != ""]), na.rm = TRUE), 0), 1),
...
cor_data <- stocks_filtered %>%
  select(RS_Rating, EPS_Rating, Comp_Rating, `%_Chg_6_Months`, 
...
ggsave("images/rs_rating_distribution.png", p1, 
...
ggsave("images/sector_performance.png", p2, 
...
ggsave("images/rs_vs_ytd_scatter.png", p3, 
...
ggsave("images/top_sectors_rs.png", p4, 
...
ggsave("images/market_cap_distribution.png", p5, 
       width = 10, height = 6, dpi = 300)

# 6.6 Correlation Heatmap
if(exists("cor_matrix") && !is.null(cor_matrix)) {
  png("images/correlation_heatmap.png", 
...
ggsave("images/performance_timeframes.png", p6, 
       width = 10, height = 6, dpi = 300)
...
ggsave("images/top20_ytd_performers.png", p7, 
       width = 12, height = 8, dpi = 300)
...
```
![](outputs/rs_rating_distribution.png)
